...
// This is necessary because the Google Maps script is loaded asynchronously
let initMap;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const planBuilder = {
        ...
        // --- INITIALIZATION ---
        init() {
            console.log("Initializing Co-Parenting Compass...");
        ...
        attachEventListeners() {
            ...
            // Add listeners for dynamic financial updates
        ...
        updateStepView() {
            ...
            this.elements.prevBtn.style.opacity = this.currentStep === 1 ? '0.5' : '1';
        ...
        validateAndSaveStep() {
            ...
            this.planData[stepId] = {}; // Clear old data for this step
            ...
            if (this.currentStep === 1) {
                ...
                childEntries.forEach(entry => {
                    ...
                    if(name) { // Only save children with names
        ...
        addChild() {
            ...
            childEntry.className = 'child-entry grid md:grid-cols-3 gap-4 p-4 border border-gray-700 rounded-lg';
        ...
        initMap() {
            const mapOptions = {
                center: { lat: -29.8587, lng: 31.0218 }, // Default to Durban
                zoom: 8,
                mapId: 'FLAMEA_MAP_STYLE' // A custom Map ID for styling
            };
            ...
            const setupAutocomplete = (inputId, parentKey) => {
                ...
                autocomplete.addListener("place_changed", () => {
                    ...
                    this.planData.step2[`${parentKey}_address`] = {
            ...
            setupAutocomplete('parentA-address', 'parentA');
            setupAutocomplete('parentB-address', 'parentB');
                    ...
                    navigator.geolocation.getCurrentPosition(position => {
                        ...
                        geocoder.geocode({ location: latlng }, (results, status) => {
                            if (status === "OK" && results[0]) {
                                ...
                                this.planData.step2[`parent${parent}_address`] = {
                                ...
                                alert("Geolocation failed: " + status);
                            }
                        });
                    });
                });
        ...
        updateMap() {
            ...
            if (this.markers.parentA) this.markers.parentA.setMap(null);
            if (this.markers.parentB) this.markers.parentB.setMap(null);
            
            const bounds = new google.maps.LatLngBounds();
            let locationsExist = 0;

            const createMarker = (addressData, label, icon) => {
                if (addressData) {
                    locationsExist++;
                    ...
                    this.markers[label.toLowerCase()] = marker;
                    bounds.extend(position);
                }
            };
            
            createMarker(this.planData.step2.parentA_address, 'A', { url: 'http://maps.google.com/mapfiles/ms/icons/blue-dot.png' });
            createMarker(this.planData.step2.parentB_address, 'B', { url: 'http://maps.google.com/mapfiles/ms/icons/red-dot.png' });
        ...
        addExpense(name = '') {
            ...
            row.innerHTML = `
                <div class="col-span-3">
                    <label class="form-label text-xs">Expense</label>
                    <input type="text" name="expense-name" class="form-input" value="${name}" placeholder="e.g., Transport">
                </div>
                <div class="col-span-2">
                     <label class="form-label text-xs">Total Monthly Amount</label>
                    <div class="relative">
                        <input type="number" name="expense-amount" class="form-input pl-8" placeholder="e.g., 1500" min="0">
        ...
        updateFinancials() {
            ...
            const percentA = totalIncome > 0 ? (incomeA / totalIncome) * 100 : 50;
            const percentB = totalIncome > 0 ? (incomeB / totalIncome) * 100 : 50;
            ...
            this.elements.financialDashboard.innerHTML = `
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                    <div class="bg-gray-800 p-4 rounded-lg">
                        ...
                        <p class="text-xs text-gray-500">(${percentA.toFixed(1)}% of income)</p>
                    </div>
                     <div class="bg-gray-800 p-4 rounded-lg">
                    ...
                    <div class="bg-gray-800 p-4 rounded-lg">
                        ...
                        <p class="text-xs text-gray-500">(${percentB.toFixed(1)}% of income)</p>
                    </div>
                </div>
                 <div class="mt-4 flex items-center gap-4">
                    <div class="w-full contribution-bar-bg flex">
        ...
        addCalendarEvent() {
            ...
            let color = '#16a34a'; // Shared
            if(owner === 'parentA') color = '#3b82f6'; // Blue
            if(owner === 'parentB') color = '#ec4899'; // Pink

            const newEvent = {
                title: `(${owner === 'parentA' ? 'A' : 'B'}) ${title}`,
                start,
                end: end || null,
                allDay: true,
        ...
        renderReviewPane() {
            ...
            const childrenHtml = step1.children.map(c => `<li>${c.name} (DOB: ${c.dob || 'N/A'})</li>`).join('') || '<li>No children listed.</li>';
            ...
            this.elements.reviewPane.innerHTML = `
                <div class="border-b border-gray-700 pb-4">
                    ...
                    <h4 class="font-semibold mt-2">Children:</h4>
                    <ul class="list-disc list-inside text-gray-400">${childrenHtml}</ul>
                </div>
                 ...
                 <div class="border-b border-gray-700 py-4">
                    ...
                    <p><strong>Parent B Income:</strong> R ${step3.parentB_income.toFixed(2)}</p>
                     <h4 class="font-semibold mt-2">Shared Expenses:</h4>
                    <ul class="list-disc list-inside text-gray-400">${expensesHtml}</ul>
                </div>
                <div class="py-4">
                    <h3 class="text-xl font-bold text-white mb-2">Schedule</h3>
                     <ul class="list-disc list-inside text-gray-400">${scheduleHtml}</ul>
                </div>
            `;
        },
        ...
        generatePdf() {
            ...
            const coverPage = `
                <div class="pdf-page pdf-cover-page">
                    <div class="pdf-content">
                        <svg width="240" height="72" viewBox="0 0 200 60"><text x="0" y="45" font-family="Roboto Slab, serif" font-size="50" font-weight="700" fill="white">Flame</text><g transform="translate(135, 0)"><path d="M18 5 L 0 50 L 8 50 C 12 42, 24 42, 28 50 L 36 50 Z" fill="#4ade80"></path></g><line x1="0" y1="58" x2="171" y2="58" stroke="#4ade80" stroke-width="2.5" stroke-linecap="round"></line></svg>
                        <h1 style="color:white; font-size: 32pt; margin-top: 2rem;">Parenting Plan</h1>
                        <p style="color:#d1d5db; font-size: 18pt;">Prepared for ${safeText(step1.parentA_name)} and ${safeText(step1.parentB_name)}</p>
                        <p style="color:#9ca3af; margin-top: 8rem;">FATHERS. LEADERS. LEGACY.</p>
                    </div>
                    <div class="pdf-footer"><span>Doc ID: FLAMEA-PP-${Date.now()}</span><span>Version: 1.0</span><span>Generated: ${new Date().toLocaleDateString('en-ZA')}</span></div>
                </div>
            `;

            // PDF Main Content
            const childrenHtml = step1.children.map(c => `<li>Full Name: <b>${safeText(c.name)}</b>, Date of Birth: <b>${safeText(c.dob)}</b></li>`).join('');
            const expensesHtml = step3.expenses.map(e => `<li>${e.name}: R ${e.amount.toFixed(2)} per month</li>`).join('');
            ...
            const mainPage = `
                <div class="pdf-page">
                     <div class="pdf-watermark"><svg width="400" height="400" viewBox="0 0 200 60"><text x="0" y="45" font-family="Roboto Slab, serif" font-size="50" font-weight="700" fill="#000">Flame</text><g transform="translate(135, 0)"><path d="M18 5 L 0 50 L 8 50 C 12 42, 24 42, 28 50 L 36 50 Z" fill="#4ade80"></path></g></svg></div>
                    <div class="pdf-header">Co-Parenting Compass Plan</div>
                    <div class="pdf-content">
                        <h1>Parenting Plan</h1>
                        <p>This Parenting Plan is made on this day, ${new Date().toLocaleDateString('en-ZA', { day: 'numeric', month: 'long', year: 'numeric' })}, between the two parties named below, in accordance with the Children's Act 38 of 2005.</p>

                        <h2>1. The Parties</h2>
                        <h3>Parent A: ${safeText(step1.parentA_name)}</h3>
                        <p><b>Residential Address:</b> ${safeText(step2.parentA_address?.text)}</p>
                        <h3>Parent B: ${safeText(step1.parentB_name)}</h3>
                         <p><b>Residential Address:</b> ${safeText(step2.parentB_address?.text)}</p>

                        <h2>2. The Children</h2>
                        <p>This plan shall apply to the following minor child/children:</p>
                        <ul>${childrenHtml}</ul>
                        
                        <h2>3. Financial Contributions</h2>
                        <p>The parents agree to contribute to the child/children's expenses on a pro-rata basis according to their respective incomes.</p>
                        <p><b>Parent A Monthly Income:</b> R ${step3.parentA_income.toFixed(2)}</p>
                        <p><b>Parent B Monthly Income:</b> R ${step3.parentB_income.toFixed(2)}</p>
                        <h3>Agreed Monthly Expenses</h3>
                        <ul>${expensesHtml}</ul>
                        <h3>Contribution Summary</h3>
                        <p>Based on the incomes and expenses listed, the monthly contribution towards these expenses is calculated as:</p>
                        <p><b>Parent A's Contribution: R ${contributionA.toFixed(2)}</b></p>
                        <p><b>Parent B's Contribution: R ${contributionB.toFixed(2)}</b></p>

                        <h2>4. Contact Schedule</h2>
                        <p>The following schedule has been agreed upon:</p>
                         <ul>${step4.events.map(e => `<li><b>${e.title}</b> starting on ${e.start}${e.end ? ` until ${e.end}` : ''}</li>`).join('') || '<li>No specific schedule events were added.</li>'}</ul>

                        <h2>5. Agreement & Signatures</h2>
                        <p>The parties agree that this plan is in the best interests of the child/children. They agree to be bound by its terms.</p>
                        <div style="margin-top: 100px; display: grid; grid-template-columns: 1fr 1fr; gap: 4cm;">
                            <div><div style="border-bottom: 1px solid black; padding-bottom: 5px;"></div><p style="margin-top: 5px;"><b>${safeText(step1.parentA_name)} (Parent A)</b></p></div>
                            <div><div style="border-bottom: 1px solid black; padding-bottom: 5px;"></div><p style="margin-top: 5px;"><b>${safeText(step1.parentB_name)} (Parent B)</b></p></div>
                        </div>
                    </div>
                    <div class="pdf-footer"><span>&copy; Flamea.org</span><span>Fathers. Leaders. Legacy.</span><span class="page-number">Page 1</span></div>
                </div>
            `;
            
            // PDF Back Cover
             const backCover = `
                <div class="pdf-page pdf-back-cover-page">
                    <div class="pdf-content">
                        ...
                        <h2 style="color: #4ade80; font-size: 18pt;">THE BUILDER'S ETHOS</h2>
                        <p style="color: #d1d5db; max-width: 80%; font-size: 10pt;">STRUCTURE. ASSEMBLE. LEGACY. ARCHITECT. TOOL-UP. INNOVATE. SUSTAIN. OBSERVE.</p>
                    </div>
                    <div class="pdf-footer"><span>https://flamea.org</span><span>Fathers. Leaders. Legacy.</span></div>
                </div>
            `;
    ...
    // The google maps script requires the callback to be on the window object
